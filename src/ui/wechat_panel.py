...
def _run_news_round_with_lock(
    ...
    if st.session_state.get("wechat_search_running"):
        raise RuntimeError("正在处理上一次搜索，请稍等。")

    st.session_state.wechat_search_running = True
    try:
        _run_news_round(
            progress,
            query_text=query_text,
...
def _render_opening_setup():
    ...
    with cols[1]:
        if st.button(
            ...
            try:
                _commit_interaction_mode(choice)
                with st.spinner("正在整理今天的新闻并拉起群聊..."):
                    _run_news_round_with_lock(
                        lambda msg: status.info(msg),
                        query_text="最新新闻 when:1d",
                        read_articles=True,
                    )
                status.empty()
                _queue_wechat_notice("今天的新闻话题已经抛进群里了。")
                _rerun_app()
            except Exception as exc:
                status.empty()
                st.warning(f"新闻话题生成失败：{exc}")
...
def render_wechat_panel():
    ...
    if submitted:
        ...
        if not query:
            st.warning("先输入要查的内容。")
        elif st.session_state.get("wechat_search_running"):
            st.info("正在处理上一次搜索，请稍等。")
        else:
            status = st.empty()
            try:
                with st.spinner("正在联网搜索并组织群聊..."):
                    _run_news_round_with_lock(
                        lambda msg: status.info(msg),
                        query_text=query,
                        read_articles=read_articles,
                ...
                status.empty()
                _queue_wechat_notice(f"已围绕「{query}」拉起群聊讨论。")
                _rerun_app()
            except Exception as exc:
                status.empty()
