...
def _clean_article_text(text: str, max_chars: int = 5000) -> str:
...
def _decode_html_payload(payload: bytes, content_type: str) -> str:
    """
    Try declared charset first; keep common Chinese fallbacks.
    """
...
def _extract_article_text_with_trafilatura(
    html: str,
    url: str = "",
    max_chars: int = 5000,
) -> str:
    try:
        ...
        return _clean_article_text(text or "", max_chars=max_chars)
    except Exception:
        return ""


def _extract_article_text_with_readability(
    html: str,
    max_chars: int = 5000,
) -> str:
    try:
        ...
        return _clean_article_text(text, max_chars=max_chars)
    except Exception:
        return ""


def _extract_article_text_with_fallback_parser(
    html: str,
    max_chars: int = 5000,
) -> str:
    try:
...
def _extract_article_text(
    ...
    text = _extract_article_text_with_trafilatura(
        html,
        url=url,
        max_chars=max_chars,
    )
    ...
    text = _extract_article_text_with_readability(
        html,
        max_chars=max_chars,
    )
    if text:
    ...
    text = _extract_article_text_with_fallback_parser(
        html,
        max_chars=max_chars,
    )
    if text:
...
def _article_method_label(method: str) -> str:
    return {
