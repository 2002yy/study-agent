...
def test_canonical_default_contract_maps_task_layers():
    research = default_contract("research")
    learning = default_contract("learn")
    conversation = default_contract("conversation")

    assert research.source_policy == "web_only"
    assert research.closure_eligibility == "research_summary"
    assert research.learning_state_enabled is False
    assert learning.closure_eligibility == "learning_summary"
    assert learning.learning_state_enabled is True
    assert conversation.source_policy == "model_only"
    assert conversation.closure_eligibility == "not_applicable"
...
def test_research_request_does_not_enter_learning_state():
    ...
    assert contract.closure_eligibility == "research_summary"
