...
| News Search | RSS / Google News / Bing News / RSSHub-style sources | 多源新闻聚合、去重、排序、来源追溯 |
| Article Extraction | `trafilatura`, `readability-lxml`, `lxml` | 新闻网页正文读取与降级解析 |
| Security | SSRF validation, redirect checks, secret scanning | 防止读取本地/内网资源，降低密钥误提交风险 |
| File Persistence | `safe_writer.py` | 临时文件写入、原子替换、覆盖前备份、PermissionError 重试 |
| Logging | Batched session logger | 会话日志批量 flush，减少频繁写盘 |
| Testing / Quality | pytest, ruff, mypy, detect-secrets, GitHub Actions | 单元测试、Lint、类型检查、密钥扫描、CI 自动检查 |
...
```text
User query
  -> RSS / News source fetch
  -> dedup + sort
  -> link resolution
  -> article text extraction
  -> LLM digest
  -> role-based group discussion
  -> source block written to chat transcript
```
...
### 3.10 CI, Testing & Packaging Guards
...
- 配置 **GitHub Actions CI**，集成 pytest、ruff、mypy、detect-secrets 和自定义打包检查，减少密钥泄露、运行产物误打包和格式回归风险。
...
- Configured **GitHub Actions CI** with pytest, ruff, mypy, detect-secrets and custom packaging guards to reduce regression, secret leakage and runtime-output packaging risks.
...
3. Role-based group chat interaction
4. News search pipeline with source tracing
5. SSRF protection for article fetching
6. Safe writer and batched session logging
7. Streamlit fragment-based UI optimization
8. CI, tests and packaging guards

---

## 6. Summary

Study Agent 的技术价值不在于“调用一个大模型 API”，而在于围绕本地学习场景补齐了长期记忆、模型路由、上下文分层、联网检索、安全边界、写入可靠性和 CI 工程化检查。
