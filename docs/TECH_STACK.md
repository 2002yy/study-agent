...
- mypy soft check（当前仍有类型债，不作为阻断条件）
...
- 实现 **本地 RAG MVP**，支持 Markdown / TXT / DOCX / PDF 加载、来源行号分块、关键词检索、本地向量原型、hybrid 检索、引用上下文拼装、来源块、Streamlit 检索面板、单人聊天和微信群互动回复注入，并提供 FastAPI health / RAG endpoints；embedding、生产向量库和更完整的来源面板仍作为后续演进。
