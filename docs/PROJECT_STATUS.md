...
> **唯一当前状态入口**  
> 更新日期：2026-07-13  
> 当前代码基线：`6c9ded6c7091f512f4c59cc8b9a157b4f701d673`
...
> **基础架构基本完成，P0 最小正确性切片已落地，G10 ResearchRun 已完成研究合同、持久化、来源评估和首个真实阶段转换。**
...
- `WebLookupRun` 已持久化 stage、research context、query attempts、selected/rejected sources、provider status、stop reason、answer confidence。
- SQLite schema 14 增加研究字段；schema 15 将历史 `running` 查询迁移为 `legacy_run_interrupted`，不再误记为“搜索为空”。
- Repository、API response 和恢复读取支持新增字段。
- 新增确定性 `SourceAssessment`：保存 title、URL、domain、source type、relevance、directness、freshness、duplicate、rejection reason 和 worth-reading。
- 缺标题且缺 URL、缺少任何来源标识、非法 URL、重复来源会进入 rejected，不进入最终 items/source block。
- 历史裸 `selected_sources` 会在 repository 读取时自动归一化为带 assessment 的统一结构。
- 正常研究轨迹不进入 warning UI；Provider 失败才产生 warning。
- Repository 使用 compare-and-set 完成 `searching -> assessing`，重复或过期转换会失败。
- 证据置信度只描述来源覆盖，不冒充事实真实性；当前分为 none/low/medium/high。
- 测试覆盖规范化、有限扩展、成功/空/全失败、来源筛选、历史来源结构、阶段 CAS、持久化恢复、schema 14/15 历史数据迁移。
...
| 领域 | 状态 | 主要缺口 |
|---|---|---|
| G10 多步联网研究 | 搜索、评估、持久化基础完成 | 网页阅读、synthesizing、预算、取消传播、retry/resume、追问复用、前端高级轨迹 |
...
| G13 证据合同 | 部分完成 | 与 Assistant claim 的对应、web/local 统一计数和前端展示 |
...
1. **继续 G10：网页阅读、synthesizing、预算、恢复和取消。**
2. 建立 G1 LearningClosureRun。
3. 补 G12 统一阶段事件、预算和取消传播。
4. 补 G14 临时资料与逐文件状态。
...
- 只读取 `worth_reading=true` 的 selected source；
- 增加 `assessing -> reading -> synthesizing -> completed/partial` 阶段转换；
- 保存已读正文摘要、阅读失败和阅读次数；
- 设定查询数、阅读数、字符数和超时预算；
- Provider/阅读失败保留已有搜索和评估结果；
- retry 从当前 durable stage 继续，而不是重新搜索；
- 为前端提供 compact 状态和高级研究轨迹，但默认不暴露低层调试信息。
...
- `ARCHITECTURE_STATUS.md`：稳定架构边界附录；
- `RAG.md`、`TECH_STACK.md`、`STATE_MODEL.md`：稳定技术参考；
- `superpowers/specs/`：设计规范；
- `superpowers/plans/`：实施记录或历史计划；
- G1–G17 roadmap：详细需求目录，不再维护进度。

## 7. 验证状态

- 新增针对研究合同、持久化、来源评估、阶段 CAS、历史来源归一化和 schema 迁移的 pytest 回归。
- CI 工作流会运行 pytest、Ruff、detect-secrets、mypy、前端测试和生产构建。
- 当前 GitHub 连接返回空 combined status，并未返回该 push commit 的 workflow run；本次不宣称远程 CI 已通过。
- 本地执行环境无法解析 GitHub 域名，不能在容器中拉取完整仓库运行全量测试；需以仓库 Actions 实际结果为准。
