...
> **唯一当前状态入口**  
> 更新日期：2026-07-13  
> 当前代码审计基线：`7ab8c3d42495032288d5a1b06df6a03052a91fd0`
...
> **基础架构基本完成，P0 最小正确性切片已落地，正在把联网研究和课后总结升级为正式、可恢复的产品闭环。**
...
| 领域 | 状态 | 主要缺口 |
|---|---|---|
| G10 多步联网研究 | 未开始正式切片 | 状态机、查询尝试、来源评估、停止原因、重试恢复、追问复用 |
...
1. **扩展现有 WebLookupRun，形成 G10 ResearchRun。**
...
当前代码切片：**G10 ResearchRun 基础数据合同**。

原则：不新增一套与 WebLookupRun 重叠的实体，而是在现有 owner 上增加：

- research context；
- canonical query 和查询变体；
- query attempts；
- selected/rejected sources；
- provider status；
- stop reason；
- answer confidence；
- 可兼容旧数据的 SQLite migration。

第一切片完成后，后续再加入多轮 planner、网页阅读、来源评估和恢复控制，而不是一次性写成不可测试的大流程。
