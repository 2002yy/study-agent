...
> **唯一当前状态入口**  
> 更新日期：2026-07-13  
> 当前代码基线：`a1af1a8c4da1bd09e035d5b30e3897d2b526e112`
...
> **基础架构基本完成，P0 最小正确性切片已落地，G10 多步联网研究已经开始第一切片。**
...
### G10 第一切片：确定性研究合同和有限查询扩展

- 新增 `ResearchContext` 与 `QueryAttempt` 纯数据合同。
- 保留 raw query，同时产生 canonical query、有限 query variants、当前日期和 freshness 信息。
- 手动 `WebLookupService` 不再只搜索一次原始文本，而是按最多三个规范化变体依次查询，命中后立即停止。
- 单次 Provider 失败可继续尝试剩余变体；只有所有变体均失败时才把 WebLookupRun 标记为 failed。
- 所有查询为空仍作为 completed + empty evidence，不升级为 confirmed absence。
- 正常研究轨迹不会伪装成 warning；正式轨迹字段等待下一切片持久化。
- 新增针对规范化、有限扩展、空结果和全 Provider 失败的回归测试。
...
| 领域 | 状态 | 主要缺口 |
|---|---|---|
| G10 多步联网研究 | 第一切片完成 | SQLite 轨迹字段、来源评估、网页阅读、停止原因持久化、重试恢复、追问复用 |
...
1. **继续扩展现有 WebLookupRun，形成 G10 ResearchRun。**
...
**G10 ResearchRun 持久化基础。**

下一步在现有 `web_lookup_runs` 上通过 SQLite migration 增加：
...
同时更新 repository、API response 和恢复测试。完成持久化后，再加入多轮 planner、网页阅读和来源评估，不一次性堆成不可测试的大流程。
