...
> 当前代码切片：G10-C3a 第四小批已补 deleted target、Provider 截断和缓存热复放，当前 9 仓库/11 case；下一小批继续补 rename 与可独立标注的失败测试正例，目标仍为 24–30 case
...
- 当前 G10-C3a 第四小批：新增 HTTPX deleted target、Study Agent #48 cache-hit 与 FastAPI request-budget truncation 三个 replay，并修复 late commit-detail budget exhaustion 未传播 `truncated` 的缺口；当前本地基线为 9 仓库/11 case、9 个 Provider replay、partial rate 0.8182、cache hit rate 0.0909、symbol F1 0.3077。聚焦回归 16 passed、GitHub 专项 101 passed、全量 pytest 790 passed，前端 49 files/172 tests 与生产构建通过，Ruff 全量、目标 mypy、package helper（893 files）和 detect-secrets 均通过。
